...
class OCBE(nn.Module):
    ...
    def __init__(
        ...
        # self.inplanes = 256 * block.expansion # = number of input channels, where block.expansion = 4 (torchvision resnet bottleneck)
        # self.layers = layers
        # num_planes = 512
        self.inplanes = 128 * block.expansion
        self.layers = 2
        num_planes = 256
        self.bn_layer = self._make_layer(block, num_planes, self.layers, stride=2)

        self.conv1_1 = conv3x3(64 * block.expansion, 128 * block.expansion, 2)
        self.bn1_1 = norm_layer(128 * block.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.conv2_1 = conv3x3(128 * block.expansion, 256 * block.expansion, 2)
        self.bn2_1 = norm_layer(256 * block.expansion)
        self.conv2_2 = conv3x3(128 * block.expansion, 256 * block.expansion, 2)
        self.bn2_2 = norm_layer(256 * block.expansion)
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(self.inplanes * self.layers, planes * block.expansion, stride),
    ...
    def forward(self, features: list[Tensor]) -> Tensor:
        ...
        if len(features) == 3:
            ...
            feature0 = self.relu(self.bn2_1(self.conv2_1(self.relu(self.bn1_1(self.conv1_1(features[0]))))))
            feature1 = self.relu(self.bn2_2(self.conv2_2(features[1])))
            feature_cat = torch.cat([feature0, feature1, features[2]], 1)
        elif len(features) == 2:
            feature0 = self.relu(self.bn1_1(self.conv1_1(features[0])))
            feature_cat = torch.cat([feature0, features[1]], 1)
